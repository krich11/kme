...
def install_kme_nginx_config(kme_cert: str, kme_key: str, ca_cert: str) -> bool:
    """Install KME nginx configuration"""
    try:
        # Convert to absolute paths for nginx
        project_root = Path.cwd()
        kme_cert_abs = str(project_root / kme_cert)
        kme_key_abs = str(project_root / kme_key)
        ca_cert_abs = str(project_root / ca_cert)
...
server {{
    ...
    ssl_certificate {kme_cert_abs};
    ssl_certificate_key {kme_key_abs};

    # CA certificate for client verification (mutual TLS)
    ssl_client_certificate {ca_cert_abs};
...
def main():
    """Main entry point"""
    if len(sys.argv) < 4:
        print("Usage: python nginx_installer.py <kme_cert> <kme_key> <ca_cert>")
        print("Example: python nginx_installer.py certs/kme_cert.pem certs/kme_key.pem certs/ca/ca.crt")
        sys.exit(1)
    ...
    if not install_kme_nginx_config(kme_cert, kme_key, ca_cert):
        sys.exit(1)
