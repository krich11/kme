...
for i in $(seq 0 $((sae_count - 1))); do
    ...
    response=$(curl -s -w "HTTPSTATUS:%{http_code}" -X GET "$KME_ENDPOINT/health/ready"         --cert "$cert_file"         --key "$key_file"         --cacert "$CA_FILE"         --connect-timeout 10)
    ...
    response=$(curl -s -w "HTTPSTATUS:%{http_code}" -X GET "$KME_ENDPOINT/api/v1/keys/$sae_id/status"         --cert "$cert_file"         --key "$key_file"         --cacert "$CA_FILE"         --connect-timeout 10)
...
if [[ -n "$master_sae" ]]; then
    ...
    for slave_sae in $slave_saes; do
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" -X POST "$KME_ENDPOINT/api/v1/keys/$slave_sae/enc_keys"             --cert "$master_cert"             --key "$master_key"             --cacert "$CA_FILE"             --header "Content-Type: application/json"             --data "$key_request"             --connect-timeout 10)
